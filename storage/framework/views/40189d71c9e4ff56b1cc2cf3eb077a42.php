...
(function () {
    ...
    const handleTouchStart = (event) => {
        ...
        // Проверяем, находимся ли мы в режиме формы измерения
        // Если да, то не обрабатываем открытие меню - это будет делать Alpine.js обработчик
        const measurementFormSection = document.getElementById('athlete-measurement-form-section');
        if (measurementFormSection && measurementFormSection.offsetParent !== null) {
            // Элемент видим, значит мы в режиме формы - не обрабатываем меню
            return;
        }
...
<div id="deleteConfirmationModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    ...
    // Основной компонент для измерений
        ...
        return {
            ...
            getSwipeTargetElement() {
                if (this.currentView === 'form') {
                    const element = document.getElementById('athlete-measurement-form-section');
                    if (element) {
                        return element;
                    }
                }
...
async function submitMeasurementForm() {
    try {
        ...
        if (response.ok && result.success) {
            ...
            // Закрываем форму и возвращаемся к списку
            const app = Alpine.$data(document.querySelector('[x-data*="measurementApp"]'));
            if (app && app.showList) {
                app.showList();
            }
...
// Функция для показа формы добавления измерения
function showMeasurementForm() {
    const appElement = document.querySelector('[x-data*="measurementApp"]');
    if (appElement) {
...
document.addEventListener('DOMContentLoaded', function() {
    window.showMeasurementForm = showMeasurementForm;
