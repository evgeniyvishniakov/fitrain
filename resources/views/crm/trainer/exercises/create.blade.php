...
@section("page-title", "Создать упражнение")
        ...
        <form method="POST" action="{{ route('crm.exercises.store') }}" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select id="category" 
                            name="category" 
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('category') border-red-500 @enderror"
                            required>
                ...
                <div>
                    <label for="equipment" class="block text-sm font-medium text-gray-700 mb-2">Оборудование *</label>
                    <select id="equipment" 
                            name="equipment" 
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('equipment') border-red-500 @enderror"
                            required>
                        <option value="">Выберите</option>
                        @foreach(\App\Models\Trainer\Exercise::EQUIPMENT as $key => $label)
                            <option value="{{ $key }}" {{ old('equipment') == $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label for="difficulty" class="block text-sm font-medium text-gray-700 mb-2">Сложность *</label>
                    <select id="difficulty" 
                            name="difficulty" 
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('difficulty') border-red-500 @enderror"
                            required>
                        <option value="">Выберите</option>
                        @foreach(\App\Models\Trainer\Exercise::DIFFICULTY as $key => $label)
            ...
            <div>
                ...
                <textarea id="description" 
                          ...
                          placeholder="Краткое описание упражнения...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
                <a href="{{ route('crm.exercises.index') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-50 border border-gray-300 rounded-lg hover:bg-gray-100 transition-colors">
                    Отмена
                </a>
                <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-colors">
                    Создать упражнение
                </button>
            </div>
        </form>
